#!/bin/bash
# backup-watchdog.sh - Мониторит папку и автоматически архивирует новые файлы/папки

# Проверяем, передан ли аргумент (путь к папке)
if [ $# -eq 0 ]; then
    echo "❌ Ошибка: Не указана папка для отслеживания."
    echo "   Использование: ./backup-watchdog.sh /путь/к/папке"
    exit 1
fi

WATCH_DIR="$1"

# Проверяем, существует ли указанная папка
if [ ! -d "$WATCH_DIR" ]; then
    echo "❌ Ошибка: Папка '$WATCH_DIR' не существует."
    exit 1
fi

echo "✅ Запуск мониторинга папки: $WATCH_DIR"
echo "📦 Новые файлы будут автоматически запакованы в .tar.gz и удалены"
echo "🛑 Для остановки нажмите Ctrl+C"
echo "---"

# Основной цикл мониторинга
while true; do
    # Ищем файлы и папки в целевой директории
    find "$WATCH_DIR" -maxdepth 1 -mindepth 1 ! -name "*.tar.gz" | while read -r ITEM_PATH; do
        # Формируем имя архива с временной меткой
        ITEM_NAME=$(basename "$ITEM_PATH")
        TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
        ARCHIVE_NAME="backup_${TIMESTAMP}_${ITEM_NAME}.tar.gz"
        ARCHIVE_PATH="$WATCH_DIR/$ARCHIVE_NAME"
        
        # Архивируем файл или папку
        echo "[$(date +'%H:%M:%S')] 📦 Архивирую: $ITEM_NAME"
        
        # Переходим в родительскую директорию для корректной архивации
        PARENT_DIR=$(dirname "$ITEM_PATH")
        BASENAME=$(basename "$ITEM_PATH")
        
        if tar -czf "$ARCHIVE_PATH" -C "$PARENT_DIR" "$BASENAME" 2>/dev/null; then
            echo "   ✅ Успешно создан архив: $ARCHIVE_NAME"
            
            # Удаляем исходный файл/папку после успешной архивации
            rm -rf "$ITEM_PATH"
            echo "   🗑️  Удалён исходный объект: $ITEM_NAME"
        else
            echo "   ❌ Ошибка при создании архива для: $ITEM_NAME"
        fi
        
        echo "---"
    done
    
    # Пауза перед следующей проверкой
    sleep 5
done